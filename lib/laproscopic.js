class Laparoscopic{
    ...
    createTarget(mesh, type, n, interaction){
        ...
        this.targetsType.push(tempObj)

        for(let i = 0; i < n; i++){
            
            var material = new THREE.MeshBasicMaterial( {color: 0xffff00} );
            var target = new THREE.Mesh( mesh.geometry, material );
            ...
            this.targetBoundingBoxes.push(bbR);
        }   
    }
    ...
    updateBoundingBoxes(){
        ...
        for(let i = 0; i < this.targetBoundingBoxes.length; i++){
            this.targetBoundingBoxes[i].bb = this.targetBoundingBoxes[i].bb.setFromObject(this.targetBoundingBoxes[i].mesh);
        }
    }
    ...
    checkCollision(){
        for(let i = 0; i < this.targetBoundingBoxes.length; i++){
           let alreadyGrabbed = false;
            if(this.boundingBoxes[0].bb.intersectsBox(this.targetBoundingBoxes[i].bb) && this.boundingBoxes[1].bb.intersectsBox(this.targetBoundingBoxes[i].bb)){
                let openingLeft = Math.floor(this.boundingBoxes[0].bb.max.y - this.boundingBoxes[0].bb.min.y * 2 );
                if( openingLeft >= 1 && openingLeft < 1.5){
                    
                    let grabbed = {
                        target: i,
                        box: this.targetBoundingBoxes[i]
                    }
                    this.toolLeft.tool.add(this.targetBoundingBoxes[i].mesh);
                    this.targetBoundingBoxes[i].mesh.position.set(0,-32,0);
                    this.toolLeft.targetGrabbed = grabbed;
                    this.toolLeft.grabbed = 1;
                    this.targetBoundingBoxes.splice(i, 1);
                    alreadyGrabbed = true;
                }
            }
    
                if(!alreadyGrabbed){
                    if(this.boundingBoxes[2].bb.intersectsBox(this.targetBoundingBoxes[i].bb) && this.boundingBoxes[3].bb.intersectsBox(this.targetBoundingBoxes[i].bb)){
                        ...
                        if( openingRight >= 1 && openingRight < 1.5){
                            ...
                            this.toolRight.tool.add(this.targetBoundingBoxes[i].mesh);
                            this.targetBoundingBoxes[i].mesh.position.set(0,-32,0);
    ...
    checkIfHold(){
        ...
        if( this.toolLeft.grabbed == 1){
            ...
            if(openingLeft > 1.5){
                ...
                this.targetBoundingBoxes.splice(this.toolLeft.targetGrabbed.target, 0, this.toolLeft.targetGrabbed.box);
        ...
        if( this.toolRight.grabbed == 1){
            ...
            if(openingRight > 1.5){
                ...
                this.targetBoundingBoxes.splice(this.toolRight.targetGrabbed.target, 0, this.toolRight.targetGrabbed.box);
